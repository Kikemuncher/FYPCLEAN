...
function FeedList(): JSX.Element {
  ...
  // Container height and width for proper aspect ratio
  ...
  // Detect client-side rendering and initialize dimensions
  useEffect(() => {
    setIsClient(true);
    updateDimensions();
  ...
  const updateDimensions = () => {
    ...
    const maxWidth = Math.min(window.innerWidth, (windowHeight * 9) / 16);
  ...
  const handleWheel = (e: React.WheelEvent<HTMLDivElement>) => {
    ...
    if (isTrackpadLike) {
      // For trackpad, accumulate delta values
      ...
      // Set a new timer to handle the end of the scrolling gesture
      scrollTimer.current = setTimeout(() => {
        // If we've accumulated enough, navigate
      ...
      // For mouse wheel - apply lock to prevent rapid scrolling
