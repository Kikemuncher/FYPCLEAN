...
// Make FeedList a named export for proper module usage
const FeedList = (): JSX.Element => {
  ...
  const handleScrollEnd = useCallback(() => {
    ...
    if (swipeProgress !== 0) {
      ...
      const threshold = containerHeight * 0.08; // Lower threshold
  ...
  const handleWheelEndEvent = useCallback(() => {
    // Reduced timing check for faster response
  ...
  // Handle wheel event for scrolling with improved sensitivity
  const handleWheel = useCallback((e: React.WheelEvent<HTMLDivElement>) => {
    ...
    // For trackpad - SIGNIFICANTLY INCREASED sensitivity
    const progressDelta = -delta * 1.8; // Much higher sensitivity
    ...
    // Clamp the progress to reasonable limits - increased maximum
    const maxProgress = containerHeight * 0.4; // Increased maximum
  ...
  // Handle touch events for mobile with higher sensitivity
  ...
  const handleTouchMove = useCallback((e: React.TouchEvent<HTMLDivElement>) => {
    ...
    const swipeDistance = diff * 2.5; // Much more sensitive
  ...
  useEffect(() => {
    ...
    // IMPROVED wheel handler with faster response time
    const wheelHandler = () => {
      ...
      // Set a faster timeout (reduced to 70ms)
      wheelTimeout.current = setTimeout(() => {
        const timeSinceLastMovement = Date.now() - lastWheelMovement.current;
...
// Make default export explicit for proper module recognition
